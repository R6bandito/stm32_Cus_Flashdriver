...
/* PVD Flag. */
static volatile uint8_t gs_lowVoltage;
...
Cus_Flash_WaitBusy( uint32_t timeout_ms )
{
	...
	#else
		static const uint32_t errorMask = FLASH_SR_PGERR_Msk | FLASH_SR_WRPRTERR_Msk;
	#endif /* DEVICE_STM32F4xx */
...
Cus_Flash_WriteBuffer( uint32_t StartAddress, uint8_t *pData, uint32_t Buffer_Size )
{
	...
	#if (DEVICE_STM32F4xx)
		/* If the Device is F4 serial. Set FLASH_CR PSIZE Bits. (x16) */
		FLASH->CR = ((FLASH->CR & ~FLASH_CR_PSIZE_Msk) | (0x01UL << FLASH_CR_PSIZE_Pos));
	#endif /* DEVICE_STM32F4xx */

	/* hWordCount is always rounded up to an even number.  */
	uint32_t hWordCount = ((Buffer_Size + 1) / 2);

	volatile uint16_t *flash = (volatile uint16_t *)StartAddress;
	for( uint32_t index = 0; index < hWordCount; index++ )
	{
		uint16_t hw;
		if ( (index == hWordCount - 1) && ((Buffer_Size & 0x01UL) != 0) )
		{
			/* Size is odd. Handle the last half-word separately. */
			hw = pData[index * 2] | 0xFF00UL;
		}
		else 
		{
			/* Normal half-word case. */
			hw = ((uint16_t)pData[index * 2] | ((uint16_t)pData[index * 2 + 1] << 8));
		}
	...
	if ( !Cus_Flash_VerifyBuffer(StartAddress, pData, Buffer_Size) )
	{
...
{
	/* Check if the parameter is valid. */
	if ( StartAddress < FLASH_BASE || StartAddress > FLASH_END_ADDR || !pData || Size == 0 )  
		return false;

	if ( !(StartAddress & 0x03UL) && !(Size % 4) && !((uint32_t)pData & 0x03UL) )
	{
		/*  4-byte aligned. Use word access for faster verification. */
		uint32_t *flash = (uint32_t *)StartAddress;
		uint32_t *pSrc = (uint32_t *)pData;
		for( uint32_t index = 0; index < (Size / 4); index++ )
		{
			if ( *flash++ != *pSrc++ )
			{
				/* Verification failed. Word mismatch detected. */
	...
	{
		/* Not 4-byte aligned. Fall back to byte-by-byte verification.  */
		for( uint32_t i = 0; i < Size; i++ )
		{
			if ( *((uint8_t *)StartAddress + i) != pData[i] )   
			{
				/* Verification failed. Byte mismatch detected. */
				Cus_FLASH_VerifyBufferFailed_Hook(StartAddress, pData, Size);
				return false;
			}
		}
...
bool 
Cus_Flash_IsValid( uint32_t StartAddress, uint32_t Size )
{
	if ( StartAddress < FLASH_BASE || StartAddress > FLASH_END_ADDR || Size == 0 )  
		return false;

	if ( !(StartAddress & 0x03UL) && !(Size % 4) )
	{
		/* 4-byte aligned. Use x32 for faster action. */
		uint32_t *flash = (uint32_t *)StartAddress;
		for( uint32_t index = 0; index < (Size / 4); index++ )
		{
			if ( *flash++ != UINT32_MAX )
			{
				/* Area is not all 0xFF (not erased). May contain existing data. Mark as unavailable and return. */
				return false;
			}
		}
	}
	else 
	{
		...
		for( uint32_t i = 0; i < Size; i++ )
		{
			if ( *((uint8_t *)StartAddress + i) != UINT8_MAX )   
			{
				return false;
			}
		}
...
Cus_Flash_PVDClr( void )
{
	...
	{
		/* Check if the parameter is valid. */
		if ( (PageAddress < FLASH_BASE) || (PageAddress >= FLASH_END_ADDR) )  
			return CUS_FLASH_PARAMETER;

		/* Check if the address is page-aligned. */
		if ( (PageAddress % CUS_FLASH_BYTE_PER_PAGE) )   
			return CUS_FLASH_PARAMETER;     

		/* Prevent Low-Voltage operation. */
		if ( gs_lowVoltage )
			return CUS_FLASH_LOW_VOLTAGE_ERR;
		...
		/* Set the page that will be erased in FLASH_AR. */
	...
	{
		/* Check if the page_index valid. */
		if ( (FLASH_BASE + page_index * CUS_FLASH_BYTE_PER_PAGE) >= FLASH_END_ADDR )   
			return 0;

		return (uint32_t)(FLASH_BASE + ( page_index * CUS_FLASH_BYTE_PER_PAGE ));
	}
	...
	{
		if ( Address < FLASH_BASE || Address > FLASH_END_ADDR )   
			return 0;
		
		if ( (Address % CUS_FLASH_BYTE_PER_PAGE) == 0 )  
		{
			/* Already a page start address. Return */
			return Address;  
		} 

		return Cus_Flash_GetPageAddress( ((Address - FLASH_BASE) / CUS_FLASH_BYTE_PER_PAGE) );
	}



	Cus_Flash_State_t 
	Cus_Flash_WritePage( Cus_Flash_PageReq_t *pPage )
	{
		/* Check if the parameter valid. */
		if ( !pPage || pPage->PageStartAddress < FLASH_BASE || pPage->PageStartAddress >= FLASH_END_ADDR )
			return CUS_FLASH_PARAMETER;

		if ( !pPage->pBuffer || !(pPage->bufSize) )
			return CUS_FLASH_PARAMETER;

		/* Cross-page writes are not allowed. */
		if ( (pPage->Offset + pPage->bufSize) >= CUS_FLASH_BYTE_PER_PAGE )
			return CUS_FLASH_NOSPACE_ERR;

		/* Prevant Low-Voltage operation. */
		if ( gs_lowVoltage )
			return CUS_FLASH_LOW_VOLTAGE_ERR;

		Cus_Flash_State_t hReturn = Cus_Flash_WriteBuffer((pPage->PageStartAddress + pPage->Offset), pPage->pBuffer, pPage->bufSize);
		if ( hReturn != CUS_FLASH_OK )
		{
			/* Write error. */
			return hReturn;
	...
	Cus_Flash_State_t 
	Cus_Flash_ReadPage( Cus_Flash_PageReq_t *pPage )
	{
		if ( !pPage || !pPage->pBuffer || !pPage->bufSize )
			return CUS_FLASH_PARAMETER;

		if ( pPage->PageStartAddress < FLASH_BASE || pPage->PageStartAddress > FLASH_END_ADDR )
			return CUS_FLASH_PARAMETER;

		/* Read must be within a single page. Cross-page reads are not supported. */
		if ( (pPage->Offset + pPage->bufSize) > CUS_FLASH_BYTE_PER_PAGE )
			return CUS_FLASH_PARAMETER;

		if ( !((pPage->PageStartAddress + pPage->Offset) & 0x03UL) && !((uint32_t)pPage->pBuffer & 0x03UL) && !(pPage->bufSize & 0x03UL) )
		{
			/* Aligned to 4-Bytes. Use x32 for faster. */
			uint32_t *pRecv = (uint32_t *)pPage->pBuffer;
			uint32_t *flash = (uint32_t *)(pPage->PageStartAddress + pPage->Offset);
			for( uint32_t index = 0; index < (pPage->bufSize / 4); index++ )
			{
				*pRecv++ = *flash++;
			}
		}
		...
		{
			for( uint32_t index = 0; index < pPage->bufSize; index++ )
			{
				pPage->pBuffer[index] = ((uint8_t *)(pPage->PageStartAddress + pPage->Offset))[index];
			}
		}

		if ( !Cus_Flash_VerifyBuffer((pPage->PageStartAddress + pPage->Offset), pPage->pBuffer, pPage->bufSize) )
		{
			/* Read verification failed. Notify upper layer and return. */
			Cus_FLASH_VerifyBufferFailed_Hook((pPage->PageStartAddress + pPage->Offset), pPage->pBuffer, pPage->bufSize);
			return CUS_FLASH_VERIFY_ERR;
		}
	...
	Cus_Flash_GetPageIndex( uint32_t Address )
	{
		...
		return (int16_t)((Address - FLASH_BASE) / CUS_FLASH_BYTE_PER_PAGE);
	}
	...
	{
		return (int16_t)( (FLASH_END_ADDR - FLASH_BASE) / CUS_FLASH_BYTE_PER_PAGE );
	}
	...
	Cus_Flash_GetRemainPages( uint32_t PageAddress )
	{
		...
		return (int32_t)( (FLASH_END_ADDR - PageAddress) / CUS_FLASH_BYTE_PER_PAGE );
	}
	...
	{
		if ( PageStartAddress < FLASH_BASE || PageStartAddress > FLASH_END_ADDR )   
			return -1;

		/* PageStartAddress must be page-aligned. */
		if ( (PageStartAddress % CUS_FLASH_BYTE_PER_PAGE) )
			return -2;
		...
		/* Requested page count exceeds remaining pages. Return. */
		if ( PagesWaitToBeErase > PagesRemain )   
			return -3;
		...
		for( uint16_t i = 0; i < PagesWaitToBeErase; i++ )
		{
			...
			{
				/* Erase error occurred during batch. Return the number of pages successfully erased. */
	...
	Cus_FlashMgr_EraseRegion( FlashMgr_Instance_t *instance )
	{
		...
		#if (DEVICE_STM32F1xx)
			int16_t startPage = Cus_Flash_GetPageIndex(instance->mgrStartAddr);
			int16_t endPage = Cus_Flash_GetPageIndex(instance->mgrEndAddr - 1);
			if ( (startPage < 0) || (endPage < 0) )
				return CUS_FLASH_ERROR;
			
			int16_t eraseCount = endPage - startPage + 1;
			if ( Cus_Flash_ErasePages(Cus_Flash_GetPageAddress(startPage), eraseCount) != eraseCount )
				return CUS_FLASH_ERROR;
		#endif /* DEVICE_STM32F1xx */
