...
Manager 采用 Append-Only 写入，逻辑删除仅标记 `validFlag` 而不释放物理空间。当区域写满（`CUS_FLASH_NOSPACE_ERR`）时，可调用内置 Compact API 自动压缩：

```c
// 用户回调：返回 true = 保留该记录，false = 丢弃
bool keep_calib_only(const FlashMgr_Record_t *rec, void *ctx) {
    (void)ctx;
    return strncmp(rec->msgDetail, "Calib", 16) == 0;
}

// 调用 Compact：筛选保留 → 备份 → 擦除 → 回写
uint8_t backup[2048];  // 用户提供临时缓冲区（栈上或全局区）
Cus_FlashMgr_Compact(&cfg_mgr, keep_calib_only, NULL, backup, sizeof(backup));
```

Compact 内部自动完成：遍历筛选 → 备份用户数据到缓冲区 → 调用 `EraseRegion` 擦除整区 → 逐条 `Append` 回写。调用者仅需提供保留规则（回调）和临时缓冲区。如需一次性清空全部记录，直接调用 `EraseRegion` 即可。
...
// 压缩回收：写满时按回调筛选保留有效记录
bool keep_wifi_only(const FlashMgr_Record_t *rec, void *ctx) {
    (void)ctx;
    return strncmp(rec->msgDetail, "wifi_cfg", 16) == 0;
}
uint8_t backup[2048];
Cus_FlashMgr_Compact(&cfg_mgr, keep_wifi_only, NULL, backup, sizeof(backup));
```
